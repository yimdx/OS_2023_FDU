...
static struct file *fd2file(int fd) {
    // TODO
    struct proc *p = thisproc();  // Assuming myproc() returns the current process
    if (fd < 0 || fd >= NOFILE || p->oftable.ofiles[fd] == 0) {
        return 0;
    }
    return p->oftable.ofiles[fd];
}
...
Inode *create(const char *path, short type, short major, short minor,
    ...
    if ((ip = namei(path, ctx)) != 0) {
        inodes.lock(ip);
        if (type == INODE_DIRECTORY && ip->entry.type != INODE_DIRECTORY) {
        ...
        if (type != INODE_DIRECTORY && ip->entry.type == INODE_DIRECTORY) {
            // File exists and is a directory but we wanted a regular file
            inodes.unlock(dp);
            inodes.unlock(ip);
            inodes.put(ctx, dp);
            inodes.put(ctx, ip);
    ...
    if (type == INODE_DIRECTORY) {
        // Increase link count for directory entries '.' and '..'       
        usize ret1 = inodes.insert(ctx, ip, ".", ip->inode_no);
        usize ret2 = inodes.insert(ctx, ip, "..", ip->inode_no);
        dp->entry.num_links++;
        inodes.sync(ctx, dp, true);
        // inodes.unlock(ip);
        // inodes.put(ctx, ip);
    }
    usize ret = inodes.insert(ctx, dp, name, ip->inode_no);
